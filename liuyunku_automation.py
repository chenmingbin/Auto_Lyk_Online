...
class LiuYunKuNavigationAutomator:
    """溜云库导航自动化器 - 整合pywinauto + playwright"""
    ...
    def get_main_navigation_items(self) -> List[Dict]:
        ...
        try:
            ...
            # 等待导航加载
            self.page.wait_for_selector("ul[data-rfd-droppable-id='nav-list']", 
                                      timeout=10000)
    ...
    def open_dropdown_menu(self, nav_item: Dict) -> bool:
        ...
        try:
            ...
            try:
                ...
                print("⚠️  等待下拉菜单超时，尝试其他方式...")
                # 备用等待方式
                time.sleep(2)
                dropdown = self.page.query_selector("div.mantine-HoverCard-dropdown[role='dialog']")
                if dropdown and "display: none" not in dropdown.get_attribute("style"):
                    print("✅ 下拉菜单已打开（备用方式）")
                    return True
    ...
    def get_subcategories(self) -> List[Dict]:
        ...
        try:
            ...
            # 等待细分项加载
            self.page.wait_for_selector("div[class*='maxClassList_max_children_class__']", 
                                      timeout=10000)
            
            category_containers = self.page.query_selector_all(
            ...
            for cat_idx, container in enumerate(category_containers):
                try:
                    ...
                    # 获取细分项
                    sub_items = container.query_selector_all("li")
                    ...
                    for item_idx, item in enumerate(sub_items):
                        try:
                            ...
                            print(f"解析细分项 {cat_idx}-{item_idx} 时出错: {e}")
                            continue
            ...
            print(f"✅ 找到 {len(all_subcategories)} 个细分项")
            ...
            for nav_item in main_navs:
                try:
                    ...
                    for sub in subcategories:
                        ...
                        if category not in nav_data["categories"]:
                            nav_data["categories"][category] = []
                        
                        nav_data["categories"][category].append({
                            "text": sub["text"],
                            "is_active": sub["is_active"],
                            "has_close_btn": sub["has_close_btn"]
                        })
    ...
    def test_specific_navigation(self, main_nav_text: str, subcategory_text: str) -> bool:
        """测试特定导航路径"""
        try:
            print(f"\n🎯 测试导航路径: {main_nav_text} > {subcategory_text}")
            ...
            # 打开下拉菜单
            if not self.open_dropdown_menu(target_nav):
            ...
            target_sub = None
            for sub in subcategories:
                if sub["text"] == subcategory_text:
                    target_sub = sub
                    break
            
            if not target_sub:
                print(f"❌ 未找到细分项: {subcategory_text}")
                return False
            
            # 点击细分项
            success = self.click_subcategory(target_sub)
            
            if success:
                print(f"✅ 导航测试成功: {main_nav_text} > {subcategory_text}")
            else:
                print(f"❌ 导航测试失败: {main_nav_text} > {subcategory_text}")
    ...
    def save_navigation_data(self, data: Dict, filename: str = "liuyunku_navigation.json"):
        """保存导航数据到文件"""
        try:
    ...
    try:
        ...
        if navigation_data["main_navigation"]:
            ...
            total_subcategories = sum(
                sum(len(cat) for cat in nav["categories"].values()) 
        ...
        for main_nav, subcategory in test_cases:
            automator.test_specific_navigation(main_nav, subcategory)
            automator.take_screenshot(f"test_{main_nav}_{subcategory}.png")
            time.sleep(3)  # 等待页面稳定
