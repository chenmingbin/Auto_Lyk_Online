...
class LiuYunKuNavigationAutomator:
    ...
    def debug_print_html_structure(self, description: str = "当前页面结构"):
        """调试：打印当前页面的HTML结构"""
        try:
            if not self.page:
                return
            ...
            # 保存到文件
            debug_file = f"debug_html_{int(time.time())}.txt"
            with open(debug_file, 'w', encoding='utf-8') as f:
                f.write(html)
            
            print(f"📄 完整HTML已保存到: {debug_file}")
            ...
            for idx, container in enumerate(containers):
                ...
                if title_elem:
                    print(f"    标题元素: {title_elem.text_content()}")
                    print(f"    标题HTML: {title_elem.inner_html()}")
                else:
                    print(f"    ❌ 未找到标题元素")
                
                # 获取所有li项
                li_items = container.query_selector_all("ul li")
                print(f"    li项数量: {len(li_items)}")
                
                for li_idx, li in enumerate(li_items):
                    text_elem = li.query_selector("span")
                    if text_elem:
                        print(f"      {li_idx + 1}. {text_elem.text_content()}")
                    else:
                        print(f"      {li_idx + 1}. ❌ 无文本")
            ...
            for div in all_divs:
                ...
                if "display: none" not in style and "visibility: hidden" not in style:
                    ...
                    if class_name and "mantine" in class_name:
                        visible_divs.append({
                            "class": class_name,
                            "text": div.text_content()[:50] if div.text_content() else ""
                        })
            
            print(f"\n  - 可见的mantine div数量: {len(visible_divs)}")
            for i, div in enumerate(visible_divs[:10]):  # 只显示前10个
                print(f"    {i + 1}. {div['class'][:80]} - {div['text']}")
    ...
    def get_all_categories_with_retry(self, max_retries: int = 3, wait_time: float = 2.0) -> List[Dict]:
        """
        获取所有大类（带重试机制）
        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                if not containers:
                    print(f"    ❌ 未找到任何容器，等待 {wait_time}秒...")
                ...
                if categories:
                    ...
                    print(f"  ⚠️  未找到有效大类，等待 {wait_time}秒后重试...")
                    time.sleep(wait_time)
    ...
    def click_category_and_get_subcategories_with_retry(self, category: Dict, max_retries: int = 3) -> List[Dict]:
        """
        点击大类触发细分项菜单，并获取细分项（带重试）
        """
        try:
            ...
            for item_idx, item in enumerate(clickable_items):
                try:
                    ...
                    if len(after_containers) > len(before_containers):
                        print(f"      ✅ 检测到新容器出现")
                        break
                    else:
                        print(f"      ⚠️  未检测到新容器，继续尝试...")
                        continue
            ...
            # 获取细分项（带重试）
            for attempt in range(max_retries):
                print(f"      获取细分项（尝试 {attempt + 1}/{max_retries}）...")
                ...
                print(f"        ⚠️  未找到细分项，等待重试...")
                time.sleep(2)
            
            print(f"    ❌ 所有重试都失败，未找到细分项")
            return []
        ...
        try:
            ...
            for nav_idx, nav_item in enumerate(main_navs):
                try:
                    ...
                    # 调试：打印HTML结构
                    self.debug_print_html_structure(f"主导航 '{nav_item['text']}' 下拉菜单打开后")
                    
                    # 步骤b: 获取所有大类（带重试）
                    categories = self.get_all_categories_with_retry(max_retries=3, wait_time=2.0)
                    if not categories:
                    ...
                    for cat_idx, category in enumerate(categories):
                        try:
                            ...
                            # 点击大类触发细分项菜单，并获取细分项（带重试）
                            subcategories = self.click_category_and_get_subcategories_with_retry(category, max_retries=3)
    ...
    def save_navigation_data(self, data: Dict, filename: str = "liuyunku_navigation_debug.json") -> bool:
        """保存导航数据到文件"""
        try:
